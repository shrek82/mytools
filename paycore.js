...
');
	...
	if(utility.getPref("helperVersion") != window.helperVersion) {
		...
		//仅顶层页面显示
		try {
			if(parent == self) utility.showOptionDialog("tabVersion");
		} catch(ex) {
			//跨域了，也是顶级
...
var utility = {
			...
			n.toJSON = typeof JSON == "object" && JSON.stringify ? JSON.stringify : function(t) {
				...
				if(i === "object") {
					...
					if(t.constructor === Date) {
						...
						return e < 10 && (e = "0" + e), o < 10 && (o = "0" + o), s < 10 && (s = "0" + s), h < 10 && (h = "0" + h), c < 10 && (c = "0" + c), u < 100 && (u = "0" + u), u < 10 && (u = "0" + u), '"' + w + "-" + e + "-" + o + "T" + s + ":" + h + ":" + c + "." + u + 'Z"'
					}
					if(t.constructor === Array) {
						for(a = [], l = 0; l < t.length; l++) a.push(n.toJSON(t[l]) || "null");
						return "[" + a.join(",") + "]"
					}
					y = [];
					for(f in t) if(r.call(t, f)) {
						if(i = typeof f, i === "number") v = '"' + f + '"';
						else if(i === "string") v = n.quoteString(f);
						else continue;
						(i = typeof t[f], i !== "function" && i !== "undefined") && (p = n.toJSON(t[f]), y.push(v + ":" + p))
					}
					return "{" + y.join(",") + "}"
				}
	...
	notify: function(msg, timeout) {
		...
		if(window.webkitNotifications) {
			if(window.webkitNotifications.checkPermission() == 0) {
				...
				if(utility.notifyObj == null) utility.notifyObj = webkitNotifications.createNotification(utility.icon, '订票', msg);
	...
	getOptionArray: function(dom) {
		...
		return $.map(dom.options, function(o) {
			return o.value;
		});
	},
	...
	showDialog: function(object, optx) {
		...
		return(function(opt) {
			...
			opt = $.extend({
				bindControl: null,
				removeDialog: this.attr("autoCreate") == "1",
				onClose: null,
				animationMove: 20,
				speed: "fast",
				fx: "linear",
			...
			this.closeDialog = function() {
				...
				obj.animate({
					"marginTop": "+=" + opt.animationMove + "px",
					"opacity": "hide"
				}, opt.speed, opt.fx, function() {
			...
			this.animate({
				...
				opt.onShow && opt.onShow.call(obj);
			})
	...
	verifySn2: function(skipTimeVerify, data) {
		...
		try {
			...
			case "NRML":
				...
				break;
			case "GROP":
				data.typeDesc = "内部版, <span style='color:blue;'>感谢您参与我们之中</span>!";
				break;
			case "DONT":
				data.typeDesc = "<span style='color:red;'>捐助版, 非常感谢您的支持</span>!";
				break;
			case "PART":
				data.typeDesc = "合作版，欢迎您的使用";
			}
	...
	getAllPassengers: function(callback) {
		...
		function loadPage() {
			utility.post("/otsweb/passengerAction.do?method=getPagePassengerAll", {
				pageSize: 10,
				pageIndex: pageIndex
			}, "json", function(json) {
				$.each(json.rows, function() {
					utility.allPassengers.push(this);
				});
...
function initNotCompleteOrderPage() {
	...
	(function() {
		...
		var flags = $.map(tagInputs, function(e, i) {
			return e.value;
		});
		$.each(flags, function() {
			$("#showTime_" + this).hide().after("<span id='status_" + this + "'>正在查询...</span>");
		});
		...
		function doCheck() {
			...
			utility.get("https://dynamic.12306.cn/otsweb/order/myOrderAction.do?method=getOrderWaitTime&tourFlag=" + flag, null, "json", function(data) {
				...
				if(data.waitTime > 0) {
					obj.css({
						"color": "red"
					}).html("排队中<br />排队数【" + (data.waitCount || "未知") + "】<br />预计时间【" + utility.getSecondInfo(data.waitTime) + "】<br />不过这时间不<br />怎么靠谱 ╮(╯▽╰)╭");
				} else {
					obj.css({
						"color": "red"
					}).html("奇怪的状态码 [" + data.waitTime + "]....");
				}
...
function initAutoCommitOrder() {
	...
	function queryQueueCount() {
		...
		utility.get("/otsweb/order/confirmPassengerAction.do?method=getQueueCount", queryLeftData, "json", function(data) {
			...
			utility.delayInvoke(null, queryLeftTickets, 2000);
		});
	}
		...
		$.ajax({
			...
			success: function(json) {
				...
				if(json.waitTime == -1 || json.waitTime == 0) {
					...
					if(json.orderId) window.location.replace("/otsweb/order/confirmPassengerAction.do?method=payOrder&orderSequence_no=" + json.orderId);
	...
	//进度提示框
	$("table.table_qr tr:last").before("<tr><td style='border-top:1px dotted #ccc;height:100px;' colspan='9' id='orderCountCell'></td></tr><tr><td style='border-top:1px dotted #ccc;' colspan='9'><ul id='tipScript'>" + "<li class='fish_clock' id='countEle' style='font-weight:bold;'>等待操作</li>" + "<li style='color:green;'><strong>操作信息</strong>：<span>休息中</span></li>" + "<li style='color:green;'><strong>最后操作时间</strong>：<span>--</span></li></ul></td></tr>");
		...
		if(pp) {
			...
			if(pseat) {
				$(".passenger_class").each(function() {
					$(this).find("select:eq(0)").val(pseat).change();
				});
			}
		}
	};
...
<tr class='fish_sep'><td style='text-align:center;' colspan='4'><a href='http://www.fishlee.net/soft/44/' target='_blank' style='color:purple;'>12306.CN 订票助手</a> | <a href='http://www.fishlee.net/soft/44/announcement.html' style='color:blue;' target='_blank'>免责声明</a> | <a href='" + utility.getUpdateUrl() + "' target='_blank' style='color:purple;'>下载新版</a> | <a style='font-weight:bold;color:red;' href='http://www.fishlee.net/soft/44/donate.html' target='_blank'>捐助作者</a> | 版本 v" + window.helperVersion + "，许可于 <strong>" + utility.regInfo.name + "，类型 - " + utility.regInfo.typeDesc + "</strong> 【<a href='javascript:;' class='reSignHelper'>重新注册</a>】</td></tr>\
	...
	$("#txtMusicUrl").change(function() {
		window.localStorage["audioUrl"] = this.value;
	});
	...
	if(window.localStorage["customTimeRange"]) {
		...
		$.each(ctrs, function() {
			addCustomeTimeRangeToList(this);
		});
	}
	...
	$("#autoRequery").change(function() {
		if(!this.checked) resetTimer();
	});
	//刷新时间间隔
	$("#refereshInterval").change(function() {
		timeCount = Math.max(5, parseInt($("#refereshInterval").val()));
	}).change();
	...
	checkTicketsQueue.push(function() {
		...
		$("td", this).each(function(i, e) {
			...
			var opt = {
				...
				tr: tr,
				index: e.index(),
				seatType: seatOptionTypeMap[e.index() - 1]
			};
	...
	(function() {
		...
		datebox.find(".date_prev").click(function() {
			var dobj = $(this).next();
			dobj.val(utility.formatDate(utility.addTimeSpan(utility.parseDate(dobj.val()), 0, 0, -1, 0, 0, 0))).change();
		});
		datebox.find(".date_next").click(function() {
			var dobj = $(this).prev();
			dobj.val(utility.formatDate(utility.addTimeSpan(utility.parseDate(dobj.val()), 0, 0, 1, 0, 0, 0))).change();
		});
	})();
...
<label><input type='checkbox' id='autoCorrentDate' checked='checked' /> 查询日期早于或等于今天时，自动修改为明天</label>\
	...
	";
		...
		autoChangeDateList.find(":checkbox").change(function() {
			var value = $.map(autoChangeDateList.find(":checkbox:checked"), function(e, i) {
				return e.value;
			}).join("|")
...
<p style='font-size:16px; font-weight:bold;color:blue;'>一定要仔细看说明啊！切记多个浏览器准备不要老想着一棵树上吊死啊！千万不要因为自动提交订单导致你订不到票啊！！这样老衲会内疚的啊！！！！</p>\
		...
		//优选逻辑
		$("#autoorder_method").val(window.localStorage["autoorder_method"] || "0").change(function() {
			window.localStorage.setItem("autoorder_method", $(this).val());
		});
		$("#autoorder_autocancel").attr("checked", (window.localStorage["autoorder_autocancel"] || "1") == "1").change(function() {
			window.localStorage.setItem("autoorder_autocancel", this.checked ? "1" : "0");
		});

		//自动预定列表
		list_autoorder = utility.selectionArea.call($("#autobookListTd"), {
			syncToStorageKey: "list_autoBookList",
			onAdd: onAutoOrderRowStyle,
			onRemove: onAutoOrderRowStyle,
			onClear: onAutoOrderRowStyle
		});
		list_blacklist = utility.selectionArea.call($("#blackListTd"), {
			syncToStorageKey: "list_blackList"
		});
		list_whitelist = utility.selectionArea.call($("#whiteListTd"), {
			syncToStorageKey: "list_whiteList"
		});
		...
		//绑定添加清空事件
		$("#btnAddAutoBook").click(function() {
			appendTrainCodeToList(list_autoorder);
		});
		$("#btnAddWhite").click(function() {
			appendTrainCodeToList(list_whitelist);
		});
		$("#btnAddBlack").click(function() {
			appendTrainCodeToList(list_blacklist);
		});
		$("#btnClearAutoBook").click(function() {
			emptyList(list_autoorder);
		});
		$("#btnClearWhite").click(function() {
			emptyList(list_whitelist);
		});
		$("#btnClearBlack").click(function() {
			emptyList(list_blacklist);
		});
		...
		utility.getAllPassengers(function(list) {
			...
			$("#passengerList").html(h.join("")).find("input").change(function() {
				...
				var user = $.map(selected, function(e) {
					return e.value;
				});
			...
			$("#autoorder").click(function() {
				refreshPasRowStyle($("#passengerList1 :checkbox"));
			});
		});
	})();
...
");
		...
		function generateProfile() {
			...
			pro.prePassenger = $.map($("#passengerList1 :checkbox"), function(e) {
				...
				return {
					...
					idtype: data.passenger_id_type_code,
					id: data.passenger_id_no
				};
			});;
		...
		function applyProfile(pro) {
			...
			$.each(pro.whiteList, function() {
				list_whitelist.add(this + '');
			});
			list_blacklist.emptyList();
			$.each(pro.blackList, function() {
				list_blacklist.add(this + '');
			});
			list_autoorder.emptyList();
			$.each(pro.autoBookList, function() {
				list_autoorder.add(this + '');
			});
...
<li style='color:green;'><strong>最后操作时间</strong>：<span>--</span></li></div>\
	...
	$("#orderForm").submit(function() {
		...
		utility.post(form.attr("action"), form.serialize(), "text", function(html) {
			...
			utility.delayInvoke(counter, function() {
				$("#orderForm").submit();
			}, 2000);
		});
	...
	function submitOrder() {
		...
		utility.post('/otsweb/order/confirmPassengerAction.do?method=' + order_type, formData.join("&") + "&randCode=" + getVcCode(), "json", function(data) {
			var msg = data.errMsg;
...
function initLogin() {
	...
	$.each(info, function(i, n) {
		html.push("<li style='list-style:disc inside;'>" + n + "</li>");
	});
	...
	function submitForm() {
		...
		if(!data["loginUser.user_name"] || !data["user.password"] || !data.randCode || data.randCode.length != 4 /* || (utility.regInfo.bindAcc && utility.regInfo.bindAcc != data["loginUser.user_name"])*/ ) return;
...
function initPayOrder() {
	...
	function getsubmitForm() {
		utility.post(formUrl, formData, "text", function(html) {}, function() {
